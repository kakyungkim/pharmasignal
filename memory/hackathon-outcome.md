...
- **스폰서 활용도가 3/4였다.** 행사 초반 안내 슬라이드에 채점 방식이 명시돼 있었다.
  "Sponsor Integration - More sponsor APIs integrated = higher score." 정성 평가가 아니라
  **개수 비례**였다. 그리고 참가자 투표에는 이 항목이 없었으니, 개수로 채점되는 이 기준은
  Top 6를 고르는 1차 관문에서만 작동했을 가능성이 높다. 우리가 걸린 바로 그 관문이다.
...
**발표 내용에서 확인한 결정적 사실:** 4팀 Exam Coach는 플랫폼 매핑 표 옆에 **"어댑터 사전검사 4/4 ·
현장 실제 호출 0/4"** 라고 스스로 적고도 2위를 했다. 우리는 세 플랫폼을 실제로 호출했고 그것을
요약표에 초록색으로 표시했으며 못 붙인 하나를 폴백으로 명시했는데 1차 관문에서 걸렸다.
**정직함은 양쪽 다 지켰다. 갈린 것은 무엇을 화면 중심에 놓았느냐다.** 저쪽은 네 플랫폼과 용도를
표로 앞세우고 한계를 주석으로 달았고, 우리는 4단계 요약표를 앞세웠는데 그 표의 한 줄이 "폴백"이었다.
**주장은 매핑으로 하고 한계는 주석으로 다는 편이, 결함을 채점되는 축 위의 한 줄로 만드는 것보다
전달에 유리하다.** 숨기라는 뜻이 아니라 배치의 문제다.
