...
| 4 · **가두기** | Nosana | 분산 GPU 추론 엔드포인트 | 밖으로 못 내보내는 텍스트는 우리 GPU 안에서만 | **없으면 규제 산업에서 도입 자체가 불가능하다** |

Bright Data는 계정에 열려 있는 제품에 따라 세 경로 중 하나를 자동으로 고른다.
하나가 막혀도 스폰서를 통째로 포기하지 않는다.

**한 문장 요약**
> 공개 데이터는 Bright Data로 찾고, Qwen이 읽어 코드를 쓰고, 그 코드를 Daytona 샌드박스에서 따져 보고, 밖에 못 내보내는 데이터만 Nosana의 GPU 안에 가둔다.
...
- **가두기 경로만은 폴백 없음** — `build_sovereign()`은 쓸 수 없으면 `None`을 돌려주고 단계를 건너뛴다. 민감 텍스트가 상용 API로 새는 것보다 기능을 포기하는 쪽이 옳다.
- **단계 이름은 사용자의 행동으로** — 찾기, 읽기, 따지기, 가두기는 담당자가 실제로 하는 일이다. 시스템의 동작(수집·판단·실행)으로 부르면 시스템 설명이 되고, 업무 동사로 부르면 업무 설명이 된다.
