...
행사 초반에 화면으로 안내된 심사 기준은 **네 가지**였다.
...
슬라이드에 그대로 쓰여 있었다. 정성으로 가늠하는 것이 아니라 붙인 개수에 비례해 점수가 올라갔다.

**두 관문이 서로 다른 방식이었다.** 최종 수상은 심사위원 채점이 아니라
...
- 모델 7종이 예외 없이 같은 코드로 막혔다. 모델별 권한 문제가 아니라 계정 상태가 막혀 있다는 뜻이다.

콘솔에서 활성화 버튼을 찾던 것이 헛수고였던 이유이기도 하다. 활성화 버튼이 콘솔에 없었고, **설문을 제출하면 스폰서가 크레딧을 넣어 주는 방식**이었다.

**결과: 해결됐다.** 스폰서가 계정에 20달러 바우처를 넣어 주자 `qwen3.8-max`가 즉시 응답했다.
키도 엔드포인트도 처음부터 정상이었고 크레딧만 없었던 것이다.
